...
from dmcore import progressione as PROG  # noqa: E402
...
class TestLaRegolaDiBase(unittest.TestCase):
    def test_modificatore_3_5(self):
        self.assertEqual([CAR.mod(x) for x in (1, 8, 9, 10, 11, 12, 19)],
                         [-5, -1, -1, 0, 0, 1, 4])

    def test_gli_array_sono_quelli_del_manuale(self):
        self.assertEqual(CAR.ARRAY_ELITE, (15, 14, 13, 12, 10, 8))
        self.assertEqual(CAR.ARRAY_STANDARD, (13, 12, 11, 10, 9, 8))

    def test_le_tabelle_vengono_da_dmcore(self):
        # la prima stesura le ricopiava, e sbagliava ranger e «minuta»
        self.assertEqual(PROG.DADO_DI_CLASSE["ranger"], 8)
        self.assertEqual(LC.TAGLIA["minuta"], 4)
        self.assertEqual(LC.TAGLIA["minuscola"], 2)


class TestIlVincoloBatteLArray(unittest.TestCase):
    def test_la_destrezza_scritta_si_legge(self):
        self.assertEqual(CAR.des_vincolata(blocco(), 3)[0], 14)
    ...
    def test_la_destrezza_si_ricava_dal_contatto(self):
        ...
        self.assertEqual(CAR.des_vincolata(t, 3)[0], 14)

    def test_la_costituzione_si_ricava_dal_bonus_scritto(self):
        self.assertEqual(CAR.cos_da_pf(blocco(dado="pf-dado: 4d8+8"), 3)[0], 14)
    ...
    def test_la_costituzione_si_ricava_dalla_media(self):
        ...
        self.assertEqual(CAR.cos_da_pf(t, 11)[0], 26)
    ...
    def test_la_guardia_respinge_i_dv_parziali(self):
        ...
        self.assertIsNone(CAR.cos_da_pf(t, 4))

    def test_la_guardia_dipende_dal_gs(self):
        self.assertTrue(LC.plausibile(8, 11))
        self.assertFalse(LC.plausibile(8, 4))
        self.assertFalse(LC.plausibile(-3, 20))
...
class TestPfDadoNonEUnArma(unittest.TestCase):
    ...
    def test_i_dv_razziali_non_si_giudicano_col_dado_di_classe(self):
        ...
        self.assertIsNone(LC.pf_dado_sospetto(t, 6))
...
class TestLaFonteVince(unittest.TestCase):
    ...
    def test_fonte_dedicata(self):
        ...
        v, come = CAR.dalla_fonte(f.name, f.read_text(encoding="utf-8"))
    ...
    def test_fonte_con_piu_creature_si_sceglie_per_nome(self):
        ...
        v, _ = CAR.dalla_fonte(f.name, f.read_text(encoding="utf-8"))
    ...
    def test_un_modificatore_al_posto_del_punteggio_si_scarta(self):
        self.assertIsNone(LC.SESTINA.search("Str 11, Dex 15, Con 14, Int 18, Wis 10, Cha -4"))
        self.assertIsNotNone(LC.SESTINA.search("Str 11, Dex 15, Con —, Int —, Wis 10, Cha 1"))

    def test_senza_regola_per_scegliere_non_si_trascrive(self):
        f = self._file("duergar-mago3-cr4.md")
        self.assertIsNone(CAR.dalla_fonte(f.name, f.read_text(encoding="utf-8")))
...
class TestLaSchedaSiLeggePrima(unittest.TestCase):
    ...
    def test_la_sestina_italiana(self):
        ...
        self.assertEqual(LC.dalla_scheda(t)[0]["For"], 31)
...
class TestLaTagliaSiTrova(unittest.TestCase):
    ...
    def test_dal_dettaglio_della_ca_e_non_dalla_prosa(self):
        ...
        self.assertEqual(LC.taglia_di(t), -1)
...
class TestLaDestrezzaDallIniziativa(unittest.TestCase):
    def test_con_iniziativa_migliorata(self):
        ...
        self.assertEqual(CAR.des_da_iniziativa(t, 7)[0], 14)

    def test_senza_elenco_di_talenti_non_si_ricava(self):
        self.assertIsNone(CAR.des_da_iniziativa(blocco() + "\niniziativa: +5\n", 9))
...
class TestIlCasoEASemeFisso(unittest.TestCase):
    ...
    def test_la_principale_non_varia(self):
        for i in range(20):
            v, _ = CAR.genera(f"g{i}-cr1.md", "brute", 1, "")
...
class TestIlTettoDeiTS(unittest.TestCase):
    ...
    def test_il_tetto_del_drago(self):
        ...
        t = LC.tetti_dai_ts("prova-cr9.md", DRAGO)
    ...
    def test_abbassa_un_valore_dell_array(self):
        ...
        v, note = CAR.genera("prova-cr9.md", "skirmisher", 9.0, DRAGO.replace("iniziativa: +5\n", ""))
        self.assertLessEqual(CAR.mod(v["Des"]), 1)
    ...
    def test_non_tocca_i_numeri_della_scheda(self):
        ...
        v, note = CAR.genera("prova-cr9.md", "skirmisher", 9.0, testo)
    ...
    def test_un_ts_derivato_non_fa_da_tetto(self):
        ...
        self.assertNotEqual(LC.tetti_dai_ts("prova-cr9.md", DRAGO), {})
        self.assertEqual(LC.tetti_dai_ts("prova-cr9.md", testo), {})

    def test_senza_composizione_non_c_e_tetto(self):
        # un TS senza una base nota non limita niente: indovinarla e' inventare
        self.assertEqual(LC.tetti_dai_ts("x-cr3.md", "ts: Temp +2, Rifl +2, Vol +2\n"), {})

    def test_il_paladino_si_salta(self):
        testo = ("```statblocco\ngs: 5\ntipo: Medium humanoid (human), Paladin 5\n"
                 "pf: 40\nts: Temp +6, Rifl +3, Vol +3\n```\n")
        self.assertEqual(LC.tetti_dai_ts("x-cr5.md", testo), {})
...
class TestLIniziativaADueCandidati(unittest.TestCase):
    ...
    def test_i_due_candidati(self):
        self.assertEqual(CAR.iniziativa_ambigua("iniziativa: +5\n"), (1, 5))

    def test_con_i_talenti_non_e_ambigua(self):
        self.assertIsNone(CAR.iniziativa_ambigua("iniziativa: +5\n**Talenti** Robustezza\n"))

    def test_il_generatore_sceglie_fra_i_due(self):
        testo = DRAGO.replace("ts: Temp +12, Rifl +8, Vol +9\n", "")
        v, _ = CAR.genera("prova-cr9.md", "skirmisher", 9.0, testo)
        self.assertIn(CAR.mod(v["Des"]), (1, 5))
