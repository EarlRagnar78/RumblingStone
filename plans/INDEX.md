...
| [RICERCA-TOOL-ESTERNI-DM](RICERCA-TOOL-ESTERNI-DM-2026-08.md) | Metodo: **cosa vale la pena prendere dal panorama dei tool per DM** (agosto 2026) — categorie, verdetto per tool con licenza verificata, e il criterio delle tre soglie per gli import futuri | ✅ completa | 100% | ⬜ **R1** trascrizione locale della sessione (whisper.cpp, MIT, offline) → bozza di verbale per `session_recap.py`: chiude C3 ma richiede un ADR (prima dipendenza da binario esterno) e la regola del consenso | decisione DM sull'ADR della dipendenza esterna |
