# IL PORTALE DELLA FORGIA ETERNA
## PARTE 3 - Piano Elementale del Fuoco (PARTE 2/2 - COMPLETAMENTO)
### Avventura D&D 3.5 - Livello 13 - Forgotten Realms

---

**Questo documento continua da PortaleForgia-P3-PianoFuoco-PARTE1.md**

---

## 7. INCONTRO 3: SALAMANDRA NOBILE E SCHIAVI (CR 12)

### **Quando e Dove**

**Posizione:** Zona 3 (Oceano Lava), isola galleggiante medio-grande (~30m diametro)  
**Tempo Viaggio:** ~75 minuti dopo ingresso Piano Fuoco  
**Setup:** Party raggiunge isola e trova campo salamandre con schiavi umanoidi

---

### **Descrizione Situazione**

**Narrazione:**

> *Dopo attraversamento tremendo oceano lava (saltando tra isole piccole, rischiando cadute multiple), raggiungete isola più grande. Roccia solidificata, 30 metri diametro, abbastanza stabile. Pensate sia luogo riposo sicuro...*
>
> *...ma vi sbagliate.*
>
> *Al centro isola, vedete ACCAMPAMENTO. Non naturale - COSTRUITO. Strutture rudimentali roccia fusa modellata: muri bassi, fucina primitiva, gabbie.*
>
> *GABBIE. Dentro: tre creature umanoidi. Nani? No... troppo sottili. Umani? Pelle colore sbagliato - grigio cenere. Uno solleva testa quando vi vede. Occhi IMPLORANO silenziosamente. È... SCHIAVO.*
>
> *E davanti fucina, corpo serpentino massiccio si volta. SALAMANDRA NOBILE.*
>
> *Descrizione creatura: 3 metri lunghezza, corpo serpente con torso/braccia umanoidi superiori. Pelle scaglie rosse incandescenti, occhi gialli fessurati come drago. Tiene lancia +1 fuoco adamantio (chiaramente forgiata qui). Indossa armatura pezzi bronzo decorata rune Ignan.*
>
> *Parla - voce sibilo profondo, ma comprensibile (Comune con accento pesante):*
>
> **SALAMANDRA:** *"Intrusi. Territorio MIO. Perché violate? Parlate rapido o diventerete cibo."*

---

### **Opzioni Party**

**Opzione A: DIPLOMAZIA (Difficile ma Possibile)**

Salamandra è intelligente (INT 14), non bestia. Può ragionare.

**Prova Diplomazia CD 20:**
- **Successo:** Salamandra accetta lasciarvi passare SE fate compito
  - **Compito:** Liberare schiavi = NO (li vuole)
  - **Compito ALTERNATIVO:** Portarle pepita mithral puro da Piano Terra (future quest)
  - **O:** Pagamento tributo (500 mo gemme/metallo ciascuno = 1.500 mo totali)
- **Fallimento:** Salamandra offesa, attacca

**Bonus Diplomazia:**
- Mostrare rispetto (inchino, parlare Ignan): +2
- Offrire oggetto metallico subito (non tributo pieno): +2
- Minacciare o insultare: Fallimento automatico

**Opzione B: COMBATTIMENTO**

Liberare schiavi richiede sconfiggere salamandra.

**Opzione C: FURTIVITÀ (Molto Difficile)**

Aggirare isola senza farsi vedere:
- Nascondersi CD 25 (tutti PG)
- Muoversi Silenziosamente CD 22
- Salamandra Osservare +11, Ascoltare +8

**Più Probabile:** Diplomazia tentativo → Se fallisce → Combattimento

---

### **STATISTICHE: Salamandra Nobile (Flamebrother)**

**Fonte:** Monster Manual pag 221, MODIFICATA (+Template Nobile, +equipaggiamento)

**Tipo:** Esterno (Fuoco, Extraplanare)  
**Taglia:** Media  
**DV:** 12d8+36 (90 HP) [Base 9d8, +3 HD template Nobile]  
**Iniziativa:** +1 (DES +1)  
**Velocità:** 6m (4 quadretti)  
**CA:** 20 (+1 DES, +6 naturale, +3 armatura bronzo)  
**Touch:** 11, Flat-Footed: 19

**Attacchi:**
- **Lancia Adamantio Fuoco +1:** +15/+10 mischia (1d8+6 più 1d6 fuoco, crit x3)
- **Coda:** +10 mischia (2d6+2 più 1d6 fuoco e aggrappo)
- **Stretta (se aggrappo):** 2d6+4 automatico per round

**Attacco Completo:**
- Lancia +15/+10 (1d8+6+1d6 fuoco) E Coda +10 (2d6+2+1d6 fuoco)

**Spazio/Portata:** 1,5m / 1,5m (lancia 3m reach)

**Attacchi Speciali:**
- **Bruciare:** Colpi fuoco richiedono TS Riflessi CD 18 o prende fuoco (1d6 fuoco/round)
- **Aggrappo Migliorato:** Coda aggancia senza provocare AoO
- **Stretta:** Una volta aggrappato, 2d6+4 danni automatici per round
- **Soffio Fuoco (1/1d4 round):** Cono 6m, 6d6 fuoco, TS Riflessi CD 18 dimezza

**Qualità Speciali:**
- **Immunità:** Fuoco
- **Vulnerabilità:** Freddo (danni x1,5)
- **DR:** 10/magia e freddo (armi non-magiche O freddo superano)
- **Resistenza Incantesimi:** RI 17
- **Scurovisione:** 18m
- **Percezione Calore:** Individua creature sangue caldo entro 18m (come scurovisione ma vs calore)

**Tiri Salvezza:**
- **Tempra:** +11 (+8 base, +3 COS)
- **Riflessi:** +8 (+8 base, +0 DES) [Errore originale MM, corretto +1 DES = +9]
- **Volontà:** +9 (+8 base, +1 SAG)

**Caratteristiche:**
- FOR 18 (+4), DES 13 (+1), COS 16 (+3), INT 14 (+2), SAG 13 (+1), CAR 13 (+1)

**Abilità:**
- Artigianato (forgiatura) +12
- Ascoltare +8
- Intimidire +8
- Osservare +11
- Raggirare +6
- Sapienza Magica +7

**Talenti:**
- Arma Focalizzata (lancia)
- Attacco Poderoso
- Incalzare
- Multiattacco (riduce penalità attacchi naturali secondari)
- Riflessi Fulminei

**Linguaggi:** Ignan (nativo), Comune

**Equipaggiamento:**
- Lancia adamantio +1 fuoco (8.000 mo valore)
- Armatura bronzo +1 (1.500 mo)
- Collana amuleto protezione +1 (1.000 mo)
- Borsa con 300 mo gemme (rubini grezzi)

**Tattiche Combattimento:**
- **Round 1:** Soffio fuoco vs gruppo (6d6 cono 6m)
- **Round 2+:** Carica su target prioritario (Artemis caster)
  - Attacca con lancia (reach 3m)
  - Se avvicina corpo a corpo, usa coda aggrappo
- **Se aggrappato nemico:** Stretta (2d6+4 automatico) + morso
- **Se HP < 30:** Ritira verso fucina, beve pozione *Cure Serious Wounds* (ha 2x)
- **Se HP < 15:** Fugge oceano lava (può immergersi senza danno, immune fuoco)

---

### **Schiavi Umanoidi - Plane-Touched (Tiefling x3)**

**Non Combattenti:** Schiavi NON aiutano combattimento (terrorizzati, malnutriti)

**Descrizione:**
- 3 Tiefling (mezzo-demoni pelle grigia, piccole corna)
- Nomi: Kazimir (♂), Ysolde (♀), Theron (♂)
- Catturati durante raid Piano Materiale anni fa
- Costretti forgiare armi per salamandre

**Se Liberati:**
- Implorano aiuto tornare Piano Materiale
- Non hanno modo tornare da soli (nessuna magia)
- Offrono ricompensa modesta (informazioni + gemme nascoste valore 200 mo)

**Informazioni Utili (Se Interrogati):**
- Descrivono Altare Tempo: "Piramide nera invertita, guardiano elementale enorme vive là"
- Avvertono: "Guardiano dorme ma sveglia se tocchi Altare senza permesso"
- Suggeriscono: "Cantare Canto Ignan Primordiale calma guardiano? Non sicuri, solo leggenda."

---

### **Mappa Tattica Incontro 3**

```
╔═══════════════════════════════════════════════════════════╗
║                                                             ║
║     INCONTRO 3 - ISOLA SALAMANDRA + SCHIAVI               ║
║          Isola Roccia Galleggiante (30m diametro)         ║
║                                                             ║
║   NORD                                                     ║
║    ↑                                                       ║
║                                                             ║
║  [OCEANO LAVA] ≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈            ║
║                                                             ║
║      ╔════════════════════════════════╗                   ║
║      ║                                 ║                   ║
║      ║  [S1] [S2] [S3]  Gabbie        ║                   ║
║      ║   (Schiavi Tiefling)            ║                   ║
║      ║                                 ║                   ║
║      ║        [F] Fucina               ║                   ║
║      ║         ▓                       ║                   ║
║      ║                                 ║                   ║
║      ║              SAL                ║                   ║
║      ║              ●   (Salamandra)   ║                   ║
║      ║                                 ║                   ║
║      ║   [M1] [M2] Muri bassi         ║                   ║
║      ║   (Copertura bassa +2 CA)      ║                   ║
║      ║                                 ║                   ║
║      ║                                 ║                   ║
║      ║           T    T    A           ║                   ║
║      ║          (H)  (D)  (R)          ║                   ║
║      ║        (PG arrivano Sud)        ║                   ║
║      ╚════════════════════════════════╝                   ║
║                                                             ║
║  [OCEANO LAVA] ≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈            ║
║                                                             ║
║   SUD                                                      ║
║    ↓                                                       ║
║                                                             ║
║  LEGENDA:                                                  ║
║  SAL = Salamandra Nobile                                   ║
║  [S1-S3] = Schiavi in gabbie (non combattenti)            ║
║  [F] = Fucina (fornisce copertura piena se dietro)        ║
║  [M1-M2] = Muri bassi (copertura bassa +2 CA)             ║
║  T(H/D) = Thorik/Tordek, A(R) = Artemis                   ║
║  ≈ = Oceano lava (morte certa se cade)                    ║
║                                                             ║
╚═══════════════════════════════════════════════════════════╝
```

**Terreno:**
- Isola roccia solida (no terreno difficile)
- Fucina: Blocca linea vista, copertura totale (+8 CA) se completamente dietro
- Muri bassi: Altezza 90cm, copertura bassa (+2 CA, +1 Riflessi)
- Bordi isola: Caduta oceano lava se spinti (morte quasi certa anche con Resist 20)

---

### **Round-by-Round Combattimento (Esempio)**

**Assumendo Diplomazia Fallita → Combattimento Inizia**

**ROUND 1:**

**Iniziativa:**
- Salamandra: 15 (DES +1, roll 14)
- Artemis: 18 (vince)
- Tordek: 16
- Thorik: 12

**Artemis:**
- Azione: Eldritch Blast vs Salamandra (distanza 18m)
- Attacco: +16 ranged touch vs Touch AC 11 = HIT automatico
- Danno: 8d6 (7d6 base +1d6 fuoco Benedizione) = ~28 danni
- Salamandra HP: 90 -28 = **62 HP**
- **Salamandra RI 17:** Artemis CL 13, check 1d20+13 vs 17 = serve 4+, ~85% successo
  - Assumendo supera RI

**Tordek:**
- Movimento: Avanzare 18m (velocità Monaco) verso Salamandra
- Azione: Raffica Colpi
  - Distanza: Raggiunge corpo a corpo
  - Attacchi: +12/+12/+7/+2 vs AC 20
    - Primo +12: Serve 8+, ~65% = HIT, danno 2d6+2 = ~9
    - Secondo +12: HIT, danno ~9
    - Terzo +7: Serve 13+, ~40% = possibile
    - Quarto +2: Serve 18+, ~15% = improbabile
  - **Totale danni:** ~18-27 HP
  - Salamandra HP: 62 -20 (media) = **42 HP**

**Thorik:**
- Movimento: Avanzare 6m (armatura pesante limita)
- Azione: Non raggiunge corpo a corpo (troppo lento)
- Azione Alternativa: Prepara azione - "Se Salamandra si avvicina, attacco"

**Salamandra (42 HP, Ferita ma Combatte):**
- **Decisione Tattica:** Eliminare minaccia primaria (Artemis caster)
- Azione: Movimento 6m verso Artemis + **Soffio Fuoco**
  - Posizionamento: Cono 6m include Artemis + possibilmente Thorik
  - Danno: 6d6 fuoco = ~21 danni
  - **Resist Fuoco 20:** Riduce a 1 danno (quasi nullo)
  - TS Riflessi CD 18 dimezza:
    - Artemis +8 Riflessi, serve 10+ = ~55% successo → 0 danni se dimezza
    - Thorik +6 Riflessi, serve 12+ = ~45% → probabilmente prende 1 danno

**Fine Round 1:** Salamandra ferita (42/90 HP), party quasi intatto

---

**ROUND 2:**

**Artemis:**
- Eldritch Blast vs Salamandra
- HIT, 8d6 = ~28 danni
- Salamandra HP: 42 -28 = **14 HP** (GRAVEMENTE FERITA)

**Tordek:**
- Raffica Colpi
- Due colpi hit = ~18 danni
- Salamandra HP: 14 -18 = **-4 HP → MORTA**

**Combattimento Finito - Round 2**

---

### **Dopo Combattimento**

**XP Assegnati:** 2.400 XP totali (600 per PG)
- Salamandra Nobile CR 9 (4.800 XP base) ÷ 2 (party 3 PG invece 4) = ~2.400 XP

**Danni Subiti Party:** Minimi (Resist Fuoco 20 nega soffio quasi completamente)

**Tesoro Salamandra:**
- Lancia Adamantio +1 Fuoco (8.000 mo) - **OTTIMO per Thorik!**
  - Può sostituire Aegis Fang temporaneamente? NO - Aegis Fang sacra, ma può vendere
- Armatura bronzo +1 Media (1.500 mo) - Vendibile
- Amuleto protezione +1 (1.000 mo) - Equipaggiabile
- Borsa gemme rubini (300 mo)
- **2x Pozioni *Cure Serious Wounds*** (750 mo valore, 3d8+5 HP ciascuna)

**Totale Valore:** 11.550 mo (eccellente!)

---

### **Liberare Schiavi**

**Se Party Apre Gabbie:**

**Narrazione:**

> *Schiavi crollano ginocchia quando aprite gabbie. Kazimir, il più vecchio (tiefling ~40 anni apparenti), piange silenziosamente. "Grazie... grazie... anni... ANNI intrappolati qui..."*
>
> *Ysolde tocca terra solida fuori gabbia con reverenza religiosa. "Credevo morire qui. Bruciare in fucina salamandra. Tu... salvato noi."*
>
> *Theron, più giovane (20 anni?), guarda piramide lontana. "Voi andare là? Altare Tempo? Pericoloso. Guardiano... terribile. Ma se sconfiggete... forse riportate noi casa?"*

**Opzioni:**

**A) Promettere Riportarli Sala Forgia Eterna (poi Piano Materiale):**
- Schiavi seguono party (rallentano movimento -50%, ma gestibile)
- **Complicazione:** Schiavi non combattenti, vulnerabili (AC 10, 10 HP)
- **Beneficio:** Gratitudine profonda, informazioni dettagliate Piramide

**B) Dire Attendere Qui Fino Ritorno:**
- Party completa missione, poi torna prenderli
- Schiavi timorosi ma accettano
- **Rischio:** Altre salamandre potrebbero trovare isola (10% chance per ora)

**C) Dare Loro Acqua + Direzione, Lasciarli Arrangiarsi:**
- Schiavi cercano tornare portale da soli
- **Probabilità Successo:** 20% (bassissima, Piano Fuoco mortale)
- Party guadagna Karma negativo (azione Caotica Neutrale tendente Male)

**Raccomandazione:** Opzione B (promettere tornare). Bilanciamento praticità vs compassione.

---

## 8. LA PIRAMIDE ROVESCIATA - ALTARE DEL TEMPO

### **Arrivo alla Piramide**

**Distanza da Isola Salamandra:** ~500m oceano lava  
**Tempo Traversata:** 20 minuti (saltare isole piccole instabili)  
**Arrivo:** Party raggiunge base piramide (piattaforma grande 60m x 60m)

---

**Narrazione:**

> *Dopo traversata estenuante finale oceano lava (rischiando cadute, esaurendo acqua rimanente), raggiungete DESTINAZIONE.*
>
> *Piramide Rovesciata Ossidiana Nera.*
>
> *È... maestosa. Terrificante. IMPOSSIBILE. Galleggia 100 metri sopra oceano lava senza supporto visibile. Punta (base originale piramide) punta GIÙ, quasi tocca superficie magma. Base (sommità originale) è SOPRA - piattaforma mastodontica 60m x 60m ossidiana perfettamente levigata.*
>
> *Superficie ossidiana riflette fiamme oceano sotto, creando illusione piramide BRUCI internamente. E incise superficie - rune. MIGLIAIA rune Ignan Primordiale, così antiche che nemmeno Aegis Fang riconosce tutte.*
>
> *Centro piattaforma: **ALTARE DEL TEMPO**.*
>
> *Cerchio 6m diametro, rune dorate disposte pattern geometrico complesso. Centro cerchio: PIEDISTALLO adamantio alto 1m, sommità VUOTA - forma incavo OVALE (dimensioni pugno). Topazio manca. Rubato? O mai esistito?*
>
> *E dormiente ACCANTO altare, corpo enorme arrotolato come serpente dormiente...*
>
> *...GUARDIANO.*

---

### **Il Guardiano - Elementale del Fuoco Anziano**

**Descrizione (Mentre Dorme):**

> *Creatura è... puro fuoco reso solido. Altezza (se eretto): 10 metri. Forma vagamente umanoide ma FLUIDA - braccia spesse come tronchi, testa corona fiamme perpetue, occhi chiusi ora (dorme) ma intuisci quando aprono saranno fornaci viventi.*
>
> *Respira (elementali respirano?): ogni esalazione, ondata calore distorce aria entro 9m. Ogni inalazione, fiamme corpo brillano più intense.*
>
> *Aegis Fang sussurra (allarmata): "Anziano. Elementale Anziano. Guardiano Altare da millenni. Portatore... questo è battaglia VERA. Preparati."*
>
> *Corona pulsa urgente: "Non sottovalutare. Anche con Benedizioni mie, creatura può uccidervi. Tattiche sagge. Cooperazione. O morte."*

---

### **Opzioni Party**

**Opzione A: COMBATTIMENTO DIRETTO (Più Probabile)**

Avvicinarsi Altare → Elementale sveglia → Boss Fight

**Opzione B: FURTIVITÀ (Quasi Impossibile)**

- Nascondersi CD 30 (Elementale Percezione Calore = vede sangue caldo automaticamente entro 18m)
- Magia *Invisibility*: NON funziona (Percezione Calore ignora invisibilità)
- Magia *Nondetection*: Funzionerebbe MA party non ha
- **Verdetto:** Impossibile evitare combattimento

**Opzione C: TENTARE COMUNICAZIONE (Interessante)**

Schiavi menzionarono "Canto Ignan Primordiale calma guardiano?"

**Procedura:**
1. PG deve conoscere/improvvisare canto Ignan (Intrattenere Canto CD 25)
2. Cantare mentre si avvicina Altare
3. **Se successo:** Elementale sveglia MA non ostile, permette rituale se PG dimostra rispetto
4. **Se fallimento:** Elementale offeso doppio, attacca con furia (+2 attacco/danni primo round)

**Problema:** Nessun PG ha Intrattenere +25 probabilmente. Artemis forse? Ring potrebbe aiutare?

**Raccomandazione DM:** Permettere tentativo, MA difficilissimo. Fallimento probabile → combattimento anyway.

---

## 9. BOSS FIGHT: ELEMENTALE DEL FUOCO ANZIANO (CR 12)

### **STATISTICHE COMPLETE: Elementale del Fuoco Anziano (Huge Elder)**

**Fonte:** Monster Manual pag 99, SIZE Enorme (Huge)

**Tipo:** Esterno (Elementale, Fuoco, Extraplanare)  
**Taglia:** Enorme (Huge)  
**DV:** 24d8+120 (228 HP)  
**Iniziativa:** +13 (DES +5, Iniziativa Migliorata +4)  
**Velocità:** 18m (12 quadretti)  
**CA:** 21 (-2 taglia, +5 DES, +8 naturale)  
**Touch:** 13, Flat-Footed: 16  

**Attacchi:**
- **Schianto:** +28 mischia (2d8+9 più 2d8 fuoco)
  - BAB +24, FOR +9, taglia -2 = +31... Errore? Corretto: +24 +9 -2 +Arma Foc = +28 OK

**Attacco Completo:** 2 schianti +28 (2d8+9 + 2d8 fuoco ciascuno)

**Spazio/Portata:** 4,5m / 4,5m (occupa 9 quadretti, reach 3 quadretti)

**Attacchi Speciali:**
- **Burn (Bruciare):** Ogni colpo richiede TS Riflessi CD 26 o prende fuoco (2d8 fuoco/round fino estinto)
- **Infuocato (Aura):** Creature entro 1,5m prendono 2d6 fuoco automaticamente ogni round (no TS)
  - Resist Fuoco 20 riduce a 0 (party protetto!)

**Qualità Speciali:**
- **Immunità:** Fuoco (ovviamente)
- **Vulnerabilità:** Freddo (x1,5 danni)
- **DR:** 10/— (qualsiasi arma magica supera)
- **Scurovisione:** 18m
- **Percezione Calore:** 18m (vede creature sangue caldo attraverso oscurità/invisibilità)

**Tiri Salvezza:**
- **Tempra:** +19 (+16 base, +3 COS)
- **Riflessi:** +19 (+16 base, +3 DES) [Errore MM: DES +5 non +3, corretto +21]
- **Volontà:** +10 (+8 base, +2 SAG)

**Caratteristiche:**
- FOR 28 (+9), DES 21 (+5), COS 16 (+3), INT 10 (+0), SAG 15 (+2), CAR 11 (+0)

**Abilità:**
- Ascoltare +21
- Intimidire +19
- Osservare +21
- Sapienza Magica +19

**Talenti:**
- Arma Focalizzata (schianto)
- Attacco Poderoso
- Incalzare
- Fendente Poderoso
- Iniziativa Migliorata
- Critico Migliorato (schianto)
- Volontà di Ferro
- Multiattacco

**Linguaggi:** Ignan

**Tattiche Boss:**
1. **Round 1 (Sveglia):** Alza, ruggisce (Intimidire vs party, TS Volontà CD 20 o Scosso -2 attacco/TS per 3 round), carica su PG più vicino (Thorik probabilmente)
2. **Round 2+:** Attacco Poderoso massimo (-5 attacco, +10 danni), Schianto doppio target prioritario
3. **Se flanked:** Fendente Poderoso (attacco cleave colpisce due nemici adiacenti)
4. **Se HP < 100:** Attacco Poderoso moderato (-3/+6)
5. **Se HP < 50:** Ritira su Altare, tenta rigenerarsi (assorbe potere Altare, +20 HP, 1 volta sola)
6. **Mai fugge:** Guardiano combatte fino morte

---

### **Mappa Tattica Boss Fight**

```
╔═══════════════════════════════════════════════════════════════╗
║                                                                 ║
║         BOSS FIGHT - ELEMENTALE FUOCO ANZIANO                  ║
║           Piattaforma Piramide (60m x 60m)                     ║
║                                                                 ║
║   NORD                                                         ║
║    ↑                                                           ║
║                                                                 ║
║                                                                 ║
║                    ╔═══════════╗                               ║
║                    ║           ║                               ║
║                    ║  ALTARE   ║  (6m diametro)                ║
║                    ║   TEMPO   ║                               ║
║                    ║     ◉     ║  Piedistallo Centro           ║
║                    ║           ║                               ║
║                    ╚═══════════╝                               ║
║                                                                 ║
║                                                                 ║
║              BOSS                                               ║
║              ████   (Elementale Anziano)                       ║
║              ████   (Huge, 4,5m x 4,5m)                        ║
║                                                                 ║
║           (Posizione iniziale: dormiente accanto Altare)       ║
║                                                                 ║
║  ─────────────────────────────── (18m distanza)                ║
║                                                                 ║
║                                                                 ║
║         T         T         A                                  ║
║        (H)       (D)       (R)   (PG Iniziano Sud)             ║
║                                                                 ║
║                                                                 ║
║   SUD                                                          ║
║    ↓                                                           ║
║                                                                 ║
║  TERRENO:                                                      ║
║  - Piattaforma ossidiana liscia (no terreno difficile)        ║
║  - Altare: Blocca movimento, fornisce copertura (+4 CA)       ║
║  - Bordi piattaforma: Caduta 100m in oceano lava (morte)      ║
║  - Aura Infuocato Boss: 1,5m raggio, 2d6 fuoco automatico     ║
║    (Resist 20 party riduce a 0)                                ║
║                                                                 ║
╚═══════════════════════════════════════════════════════════════╝
```

---

### **Round-by-Round Boss Fight (Tattica Ottimale)**

**ROUND 1 - SVEGLIA**

**Setup:** Party avanza verso Altare (18m distanza). Elementale SVEGLIA quando entro 12m.

**Narrazione Sveglia:**

> *Quando Thorik attraversa linea invisibile 12 metri da Altare, accade. Elementale APRE OCCHI. Non occhi - FORNACI. Fuoco bianco puro brucia dove dovrebbero essere pupille. Alza corpo titanico - 10 metri altezza ora, torreggia su voi come montagna vivente fuoco.*
>
> *E RUGGISCE.*
>
> *Suono non è voce - è ERUZIONE. Ondata sonora + calore vi investe. Orecchie fischiano. Pelle pizzica anche con Benedizioni.*
>
> *In Ignan antico (Thorik capisce vagamente): "INTRUSI! ALTARE SACRO! MORTE!"*
>
> *Iniziativa.*

**Iniziativa:**
- Elementale: 25 (DES +5, Iniz.Migl +4, roll 16)
- Artemis: 18
- Tordek: 16
- Thorik: 12

**Elementale (Primo Turno):**
- **Intimidire (Azione Gratuita):** Ruggito Terrificante
  - Tutti PG: TS Volontà CD 20 o Scosso -2 attacco/TS per 3 round
  - Thorik Volontà +7 (+4 Trinità se attiva) = serve 13+ (40-70%)
  - Tordek Volontà +10 (+4 Trinità) = serve 6+ (75-95%)
  - Artemis Volontà +10 (+4 Trinità) = serve 6+ (75-95%)
  - **Assumendo:** Thorik fallisce (Scosso), Tordek e Artemis successo
- **Movimento:** Carica 36m (corsa x2) verso Thorik (più vicino)
  - Attacco carica: +30 (base +28, +2 carica) vs AC 21 (Thorik)
    - Serve 11+ (50% base, ma Thorik Scosso riduce AC? No, Scosso penalizza attacchi attaccante, non CA)
    - HIT probabile
  - Danno: 2d8+9+2d8 fuoco+2 carica = 4d8+11 = ~30 danni
    - Resist Fuoco 20: Riduce 2d8 fuoco (~9) a 0? NO - 2d8 media 9, -20 = negativo, trattato 0
    - **Danno effettivo:** 2d8+11 = ~20 danni contundenti
  - **Burn:** Thorik TS Riflessi CD 26 o prende fuoco
    - Thorik Riflessi +6 (+4 Trinità) = +10, serve 16+ = ~25% successo
    - Probabilmente PRENDE FUOCO (2d8 fuoco/round)
    - **MA** Resist Fuoco 20: Riduce 2d8 (~9) a 0 = NO danno fuoco continuato!
  - Thorik HP: 145 -20 = **125 HP**

**Artemis:**
- **Decisione:** Attaccare Boss O attivare Trinità?
  - **Trinità:** Azione swift condivisa con Thorik (entrambi dichiarano)
  - Beneficio: +4 TS, +2 CA, immunità paura/charme
  - **Scelta:** Attivare Trinità (aiuta Thorik Scosso, +4 TS compensa -2)
- **Azione Swift:** Dichiarare Trinità (Thorik conferma)
- **Azione Standard:** Eldritch Blast vs Elementale
  - Attacco: +16 ranged touch vs Touch AC 13 = HIT facile
  - **RI 10:** Elementale NO RI (errore mio sopra, Anziano no RI)
  - Danno: 8d6 = ~28 danni
  - Elementale HP: 228 -28 = **200 HP**

**Tordek:**
- **Movimento:** 18m (Monaco veloce) raggiunge Elementale
- **Azione:** Raffica Colpi
  - Attacchi: +12/+12/+7/+2 vs AC 21
  - **Problema:** Aura Infuocato (1,5m) = Tordek prende 2d6 fuoco automatico entrando
    - Resist 20: Riduce ~9 a 0 = OK
  - **Colpi:**
    - Primo +12: Serve 9+ = ~60%, HIT, danno 2d6+2 = ~9
    - Secondo +12: HIT, ~9
    - Terzo +7: Serve 14+ = ~35%, possibile
  - **Totale:** ~18-27 danni
  - Elementale HP: 200 -20 (media) = **180 HP**

**Thorik (Scosso -2 attacco, MA Trinità compensa +4 TS):**
- **Posizione:** Corpo a corpo con Boss (caricato su lui)
- **Azione:** Attivare Eco Eroi (azione swift) + Attacco Poderoso
  - **Eco Eroi:** +2 morale attacco, +1d6 danni, 5 round
  - **Attacco Poderoso:** -2 attacco, +2 danni (moderato)
  - **Attacco:** +21 base -2 Scosso +2 Eco -2 Power = +19 vs AC 21
    - Serve 2+ = ~95% HIT
  - **Danno:** 1d10+3d6(base+fuoco+sacro)+1d6(Eco)+7+2(Power) = 1d10+4d6+9 = ~32 danni
  - Elementale HP: 180 -32 = **148 HP**
- **Aura Infuocato:** Thorik prende 2d6 fuoco automatico (Resist riduce a 0)

**Fine Round 1:** Elementale 148/228 HP (~65%), Party quasi intatto

---

**ROUND 2:**

**Elementale (148 HP):**
- **Attacco Completo:** 2 schianti su Thorik (minaccia principale)
  - **Attacco Poderoso:** -5 attacco, +10 danni (massimo)
  - Primo Schianto: +28 -5 = +23 vs AC 21 (Thorik, Trinità +2 = 23)
    - Serve 0+ = ~100% HIT
    - Danno: 2d8+9+10+2d8 fuoco = 4d8+19 = ~40 danni
    - Resist: Riduce 2d8 fuoco a 0 = 2d8+19 = ~28 danni
  - Secondo Schianto: +23 vs 23
    - Serve 0+ = HIT
    - Danno: ~28 danni
  - **Totale:** ~56 danni a Thorik
  - Thorik HP: 125 -56 = **69 HP** (CRITICO!)

**Artemis:**
- Eldritch Blast
- Danno: 8d6 = ~28
- Elementale: 148 -28 = **120 HP**

**Tordek:**
- Raffica Colpi + **Colpo Stordente** (tentativo)
  - Primo colpo: HIT, danno 2d6+2 = ~9
    - **Colpo Stordente:** Elementale TS Tempra CD 19 o stordito 1 round
    - Elementale Tempra +19: Serve 0+ = automatico successo (immune praticamente)
  - Altri colpi: ~18 danni totali round
  - Elementale: 120 -18 = **102 HP**

**Thorik (69 HP - CRITICO!):**
- **Decisione:** Continuare attaccare O curare?
  - Vigore Forgia: 1 carica, 3d8+10 = ~24 HP (azione standard)
  - Aegis Fang *Cure Moderate*: 2d8+13 = ~22 HP (azione standard)
  - **Scelta:** Usare Vigore Forgia (più potente)
- **Azione Standard:** Vigore Forgia
  - Cura: 3d8+10 = assumendo ~24 HP
  - Thorik HP: 69 +24 = **93 HP** (meglio ma ancora sotto 50%)
- **NO attacco questo round**

**Fine Round 2:** Elementale 102/228 (~45%), Thorik 93/145 (~64%)

---

**ROUND 3:**

**Elementale (102 HP, sotto 50% → Ritira su Altare):**
- **Tattica Cambio:** Movimento 18m verso Altare
- **Azione Standard:** Assorbe potere Altare (capacità speciale guardiano)
  - Cura: 20 HP
  - Elementale HP: 102 +20 = **122 HP**
- **Nota:** Questo dà 1 round respiro party

**Artemis:**
- **Movimento:** Avvicina (entro 30m Altare)
- **Azione:** Eldritch Blast
- Danno: 8d6 = ~28
- Elementale: 122 -28 = **94 HP**

**Tordek:**
- **Movimento:** Segue Elementale (18m)
- **Azione:** Raffica Colpi
- Danni: ~20
- Elementale: 94 -20 = **74 HP**

**Thorik:**
- **Movimento:** Avanza verso Elementale (6m)
- **Azione:** Attacco singolo
  - Attacco: +19 (Scosso finito ora, Eco ancora attivo)
  - Danno: 1d10+4d6+9 = ~32
- Elementale: 74 -32 = **42 HP** (CRITICO per Boss!)

**Fine Round 3:** Elementale 42/228 (~18% - CRITICO!)

---

**ROUND 4 - FINALE:**

**Elementale (42 HP - Disperato):**
- **Attacco Suicida:** Attacco Poderoso MASSIMO (-8 attacco, +16 danni)
  - Target: Tordek (più vicino)
  - Due schianti: +28 -8 = +20 vs AC 27 (Tordek)
    - Serve 7+ = ~70% uno hit, ~49% entrambi
    - **Assumendo 1 HIT:**
      - Danno: 2d8+9+16+2d8 = 4d8+25 = ~45 danni
      - Resist: -2d8 (~9) = 2d8+25 = ~34 danni
      - Tordek HP: 110 -34 = **76 HP**

**Artemis:**
- Eldritch Blast
- Danno: 8d6 = ~28
- Elementale: 42 -28 = **14 HP** (QUASI MORTO!)

**Tordek (76 HP):**
- Raffica Colpi
- Primo colpo: 2d6+2 = ~9
- Elementale: 14 -9 = **5 HP**
- Secondo colpo: ~9
- Elementale: 5 -9 = **-4 HP → MORTO**

**Elementale Anziano SCONFITTO - Round 4**

---

### **Dopo Boss Fight**

**XP Assegnati:** 3.000 XP totali (750 per PG)
- Elementale Anziano CR 12 solo (9.600 XP) ÷ 3 PG = ~3.000 XP

**Danni Subiti Party:**
- Thorik: 145 → 93 HP (~64%)
- Tordek: 110 → 76 HP (~69%)
- Artemis: 70 → 70 HP (100% - ranged sicuro!)

**Risorse Spese:**
- Thorik: 1 carica Vigore Forgia (2 rimaste)
- Thorik: Eco Eroi usato (1/combattimento, speso)
- Trinità Divina: Usata (cooldown 24 ore)

**Tesoro Boss:**
- Elementale dissolvendosi lascia **Nucleo Fuoco Puro** (gemma rubino gigante 3.000 mo)
- Nucleo può alimentare forgia magica o vendere

---

## 10. IL RITUALE DEL TOPAZIO DEL TEMPO

### **Dopo Sconfitta Guardiano**

**Narrazione:**

> *Elementale collassa, corpo fuoco dissolve come falò estinto da pioggia improvvisa. Fiamme si disperdono, lasciano solo cenere brillante e gemma rubino pulsante (Nucleo). Silenzio cade piattaforma - primo silenzio VERO da quando entrasti Piano Fuoco. No crepitio. No ruggiti. Solo... pace temporanea.*
>
> *E Altare Tempo davanti voi. Piedistallo adamantio, incavo vuoto aspetta.*
>
> *Aegis Fang canta dolcemente: "Ben fatto, portatori. Guardiano vinto. Ora... rituale. Manifestare Topazio."*
>
> *Corona pulsa: "Topazio non esiste fisicamente qui. È... possibilità. Dovete FORGIARLO da essenza Piano Fuoco stesso. Rituale complesso. Ascoltatemi."*

---

### **Procedura Rituale (Step-by-Step)**

**Durata Totale:** 20 minuti (200 round)  
**Partecipanti:** Thorik (primario - Corona), Tordek + Artemis (assistenti)  
**CD Prove:** Variabile (vedi sotto)  

---

**STEP 1: Purificare Altare (5 minuti)**

**Chi:** Thorik  
**Azione:** Recitare preghiera Moradin mentre cammina attorno Altare 7 volte (numero sacro nanico)  
**Prova:** Conoscenze (Religioni) CD 15  
**Successo:** Altare brilla oro, rune attivano  
**Fallimento:** Rituale rallenta (+5 minuti), riprova permessa  

**Narrazione Se Successo:**

> *Thorik, reciti preghiera antica - parole che generazioni nani hanno cantato da millenni. Mentre cammini settimo giro, rune Altare RISPONDONO. Brillano oro liquido, formano pattern che sembrano... respirare. Aria attorno Altare trema, realtà stessa si PIEGA leggermente.*

---

**STEP 2: Canalizzare Energia Elementale (10 minuti)**

**Chi:** Artemis (Ring of Chaotic Illumination - connessione energie caotiche)  
**Azione:** Usare Ring assorbire energia fuoco circostante, canalizzarla Altare  
**Prova:** Concentrazione CD 20 (difficile - energia violenta)  
**Successo:** Energia fluisce controllata  
**Fallimento:** Sovraccarico - Artemis prende 3d6 fuoco (Resist riduce), riprova con +2 CD  

**Narrazione Se Successo:**

> *Artemis, Ring pulsa SELVAGGIAMENTE - cicla colori oro/argento/viola così veloce diventa BIANCO puro. Allunghi mano verso oceano lava sotto piramide. E... TIRI. Come pescatore tira rete, tiri ENERGIA PURA da Piano Fuoco. Fiamme invisibili fluiscono verso Ring, poi da Ring verso Altare. Doloroso - senti calore interno - ma Benedizioni proteggono. Rune Altare assorbono energia, brillano sempre più intenso.*

---

**STEP 3: Stabilizzare Matrice Temporale (5 minuti)**

**Chi:** Tordek (Monaco - controllo ki = controllo flussi energia)  
**Azione:** Meditare centro Altare, sincronizzare ki personale con energia canalizzata  
**Prova:** Concentrazione CD 18  
**Successo:** Matrice stabilizza, cristallizza  
**Fallimento:** Feedback temporale - tutti invecchiano 1d4 anni (cosmetic), riprova permessa  

**Narrazione Se Successo:**

> *Tordek, siedi posizione loto perfetta centro Altare. Chiudi occhi. Respiri rallentano. Senti... PULSAZIONE. Altare BATTE come cuore. E ki tuo batte stesso ritmo. Sincronia. Armonia. Monaco e Altare diventano UNO per momento eterno.*
>
> *E attorno, energia canalizzata da Artemis inizia... SOLIDIFICARSI. Non più invisibile - ora VISIBILE. Fili dorati girano, intrecciano, formano pattern geometrico tridimensionale sopra piedistallo.*

---

**STEP 4: Forgiare Topazio (Finale - Tutti e 3)**

**Chi:** Thorik + Tordek + Artemis simultaneamente  
**Azione:** Thorik tocca Corona a piedistallo, Tordek mantiene meditazione, Artemis canalizza energia finale Ring  
**Prova:** Prova coordinata (tutti e 3 tirano Volontà CD 15, almeno 2 devono succedere)  
**Successo:** Topazio manifesta  
**Fallimento:** Energia collassa, danno feedback 2d6 forza tutti, devono ricominciare Step 2  

**Narrazione Successo FINALE:**

> *Thorik, posi Corona contro piedistallo. Tordek, mantieni meditazione assoluta. Artemis, Ring esplode luce finale.*
>
> *E...*
>
> *LUCE ACCECANTE.*
>
> *(Pausa drammatica 5 secondi silenzio)*
>
> *Quando vista torna, VEDETE.*
>
> *Centro piedistallo, incavo non più vuoto. Dentro: GEMMA.*
>
> *Topazio. Ma non topazio normale. Topazio che CONTIENE tempo stesso. Guardandolo, vedete riflessi IMPOSSIBILI:*
> - *Voi stessi neonati*
> - *Voi stessi anziani*
> - *Voi stessi MAI NATI*
> - *Voi stessi ETERNAMENTE ESISTENTI*
>
> *Dimensioni: Pugno adulto, forma ovale perfetta. Colore: Oro-arancio che pulsa luce interna. E quando Thorik lo tocca...*
>
> *...Corona REAGISCE. Incavo superiore Corona (fronte) brilla. Topazio VOLA da piedistallo, si INSERISCE automaticamente in Corona. CLICK. Fusione perfetta. Come se fosse sempre stato parte.*
>
> *E sentite POTERE. Topazio attivato. Prima gemma recuperata. Due rimangono.*

---

### **Effetti Topazio del Tempo (Attivato in Corona)**

**Poteri Sbloccati:**

**1. Visione Temporale (Passiva):**
- Thorik occasionalmente vede "echi" eventi passati luoghi visita
- Esempio: Cammina campo battaglia, vede fantasmi combattere (eventi 100 anni fa)
- **Uso Narrativo:** DM può rivelare informazioni background attraverso visioni

**2. Rallentare Tempo (1/giorno, azione swift):**
- Per 3 round, Thorik agisce come sotto *Haste*:
  - +1 attacco completo extra (non iterativo)
  - +1 schivare AC
  - +1 Riflessi
  - Velocità +9m
- **Non affecta:** Altri (solo Thorik)

**3. Prerequisito Viaggio Temporale:**
- Topazio necessario per attivare Affresco A6 (Viaggio Anno -1000)
- Senza Topazio: Affresco inerte
- Con Topazio: Affresco diventa portale funzionante

**Corona Aggiornata (1/3 Gemme):**
- **Nuova Apparenza:** Gemma oro-arancio brilla fronte (prima vuoto)
- **Potere Aumentato:** +1 bonus tutti poteri Corona esistenti
- **Countdown Hammerfist:** Topazio permette "sentire" urgenza (Thorik sa esattamente quante ore rimangono = meta-knowledge giustificato)

---

## 11. RICOMPENSE E RITORNO ALLA SALA

### **Dopo Rituale Completato**

**Narrazione:**

> *Topazio fuso in Corona pulsa caldo (non doloroso). Aegis Fang canta TRIONFANTE: "PRIMA PROVA COMPLETA! Portatore, ben fatto! Ora... tornare. Riposare. Preparare prossima."*
>
> *Guardate attorno. Piattaforma piramide... cambia. Altare Tempo, scopo completato, inizia DISSOLVERE. Rune dorate sbiadiscono. Piedistallo adamantio si liquefa lentamente, scorre via come mercurio.*
>
> *E distante, vedete PORTALE. Non c'era prima. Ora SÌ. Cerchio dorato identico a quello arrivo, sospeso 3m sopra bordo piattaforma. Via ritorno Sala Forgia Eterna.*
>
> *Tempo lasciare Piano Fuoco. Missione completa.*

---

### **Recuperare Schiavi (Se Opzione B Scelta Prima)**

**Se Party Promesse Tornare:**

**Procedura:**
1. Attraversare portale ritorno → Arrivano Sala Forgia Eterna
2. Riposare breve (1 ora)
3. Ri-attraversare portale Piano Fuoco
4. Navigare (Aegis Fang guida) verso isola salamandra (~30 minuti)
5. Raccogliere schiavi
6. Tornare Sala definitivamente

**Totale Tempo:** +2 ore countdown Hammerfist (accettabile se party efficiente)

**Schiavi Riconoscenza:**
- Kazimir dona **Anello Protezione +1** (famiglia possedeva, unica cosa valore)
- Ysolde condivide **mappa grezza Piano Terra** (disegnata memoria, +2 Navigazione Piano Terra)
- Theron offre servigi futuri (contatto Piano Materiale, può aiutare ricerche/acquisti)

---

### **Ritorno Sala Forgia Eterna**

**Attraversamento Portale Ritorno:**

> *Attraversate portale. Transizione INVERSA - da calore opprimente a... fresco. Freddo quasi. Temperatura Sala Forgia sembra GELIDA dopo Piano Fuoco, anche se è normale 20°C.*
>
> *Atterrate piattaforma spawn Sala. Affresco Ovest (A4 - Portale Fuoco) dietro voi... cambia. Non più portale liquido ondeggiante. Ora: AFFRESCO STATICO mostrante VITTORIA vostra.*
>
> *Immagine dipinta superficie mostra: Voi tre combattete Elementale Anziano. Momento epico - Thorik Aegis sollevata, Tordek Devastation Gauntlets brillanti, Artemis Eldritch Blast lancio. E background: Topazio manifesta Altare Tempo. È... memoria visiva conquista.*
>
> *Affresco completo. Portale Fuoco CHIUSO permanentemente. Mai più accessibile (non necessario).*

---

**Affresco A5 (Est) - Portale Terra APPARE:**

> *E mentre osservate Affresco Fuoco chiuso, sentite... vibrazione. Girate. Affresco EST - quello che era VUOTO Parte 2 - ora ESISTE.*
>
> *Materializza davanti occhi (30 secondi processo): Cornice oro/adamantio, poi superficie, poi immagine.*
>
> *Vedete Piano Elementale Terra:*
> - *Cristalli giganti grandi torri*
> - *Gravità distorta - rocce galleggiano*
> - *Geodi brillano luce interna*
> - *Centro lontano: Tempio Mithral cubo perfetto*
>
> *Portale ATTIVO. Prossima destinazione. Ma... non ora. Party esausto.*

---

### **Long Rest Raccomandato**

**Moradin Voce (attraverso Corona/Aegis):**

> *"Figli. Ben fatto Piano Fuoco. Ma stanchi. Ferite. Risorse esaurite. RIPOSATE. Sala è sicura. Tempo passa, sì - Hammerfist countdown continua - ma party morto aiuta nessuno. Otto ore. Dormite. Recuperate. POI affrontate Terra."*

**Benefici Long Rest (8 Ore):**
- HP completi (cure naturali + Benedizioni)
- Incantesimi/capacità giornaliere reset
- Vigore Forgia: 3 cariche rinnovate
- Aegis Fang: *Cure Moderate* 3/giorno reset
- Devastation Gauntlets: 4 cariche reset
- Benedizioni Moradin: Rinnovate (pregare Altare 10 min)
- **Countdown Hammerfist:** -8 ore (4 giorni 10 ore → 4 giorni 2 ore) [Ancora gestibile!]

**Durante Riposo:**
- Tordek sogna Hella (comunicazione spirituale, Hella felice progresso)
- Thorik vede visioni Topazio (flash Anno -1000, anticipa Parte 5)
- Artemis Ring pulsa gentilmente (Lathander/Mask soddisfatti)

---

## 12. TESORI TOTALI PARTE 3

### **Riepilogo Tesoro Raccolto**

| Fonte | Oggetto/Gemme | Valore (mo) | Note |
|-------|---------------|-------------|------|
| Elementali Piccoli x4 | Cenere Elementale | 200 | Componente incantesimi fuoco |
| Mephit Magma x2 | Cristalli Magma | 200 | Focus incantesimi +1 CL fuoco |
| Salamandra Nobile | Lancia Adamantio +1 Fuoco | 8.000 | Equipaggiabile/vendibile |
| Salamandra Nobile | Armatura Bronzo +1 | 1.500 | Vendibile |
| Salamandra Nobile | Amuleto Protezione +1 | 1.000 | Equipaggiabile |
| Salamandra Nobile | Gemme Rubini | 300 | Vendibili |
| Salamandra Nobile | Pozioni *Cure Serious* x2 | 750 | Consumabili |
| Elementale Anziano | Nucleo Fuoco Puro | 3.000 | Gemma/componente magico |
| Schiavi (se liberati) | Anello Protezione +1 | 1.000 | Da Kazimir |
| Schiavi | Mappa Piano Terra | — | Bonus +2 Navigazione |
| **TOTALE** | | **15.950 mo** | Eccellente! |

**Divisione (Se Party Divide Equamente):**
- 15.950 ÷ 3 = ~5.316 mo per PG

**Oggetti Equipaggiabili:**
- **Lancia Adamantio +1 Fuoco:** Thorik potrebbe usare? NO - Aegis Fang migliore (sentimentale + poteri). VENDERE.
- **Amuleto Protezione +1:** Sostituire amuleti esistenti? Thorik/Artemis potrebbero.
- **Anello Protezione +1 (da Kazimir):** Utile backup.

---

## 13. XP TOTALI PARTE 3

### **Riepilogo XP Assegnati**

| Incontro | CR | XP Totali | XP per PG (÷3) |
|----------|----|-----------| ---------------|
| Pattuglia Elementali Piccoli x4 | 10 | 1.200 | 400 |
| Ponte Lava + Mephit x3 | 11 | 1.800 | 600 |
| Salamandra Nobile | 9 | 2.400 | 800 |
| Elementale Fuoco Anziano | 12 | 3.000 | 1.000 |
| Rituale Topazio (story award) | — | 600 | 200 |
| **TOTALE** | | **9.000 XP** | **3.000 XP/PG** |

**Progressione Livello:**
- Livello 13 → 14: Richiede 91.000 XP
- Party probabilmente aveva ~78.000 XP pre-avventura
- Dopo Parte 1-2-3: +3.000 (Parte 3) + probabilmente ~2.000 (Parti 1-2) = +5.000 XP totali
- **Nuovo Totale:** ~83.000 XP (ancora livello 13, ma progredendo)

**Nota:** Party raggiungerà livello 14 probabilmente DOPO Piano Terra (Parte 4) o durante Battaglia Finale (Parte 6).

---

## 14. NOTE PER IL DM

### **Pacing Parte 3**

**Durata Totale Stimata: 3-4 ore**

| Segmento | Tempo | Note |
|----------|-------|------|
| Arrivo + Descrizione Ambiente | 15 min | Immersione atmosferica importante |
| Zona 1 + Incontro 1 (Elementali) | 30 min | Primo combattimento, impostare tono |
| Zona 2 + Incontro 2 (Ponte Lava) | 45 min | Skill challenge + combattimento |
| Zona 3 + Incontro 3 (Salamandra) | 45 min | Roleplay diplomazia possibile |
| Piramide + Boss Fight | 60 min | Climax Parte 3, combattimento epico |
| Rituale Topazio | 20 min | Momento ricompensa, non affrettare |
| Ritorno + Riposo | 15 min | Transizione Parte 4 |

**Totale:** ~3,5 ore

---

### **Bilanciamento Difficoltà**

**Benedizioni Moradin Rendono Party Forte vs Fuoco:**
- Resist Fuoco 20 nega MAGGIORANZA danni elementali
- Immunità calore ambientale elimina attrition

**COMPENSAZIONI Applicate:**
- Elementali hanno +2 DV (più HP)
- Boss ha tattica aggressiva (Attacco Poderoso massimo)
- Hazards ambientali ancora pericolosi (cadute lava)
- Numero incontri aumentato (4 invece 3 standard)

**Risultato:** Parte 3 è **CHALLENGING ma FAIR** per party livello 13 con Benedizioni.

---

### **Adattamenti se Party Struggling**

**Se Party HP Critici Pre-Boss:**
1. Moradin interviene: Benedizione extra +1 uso Vigore Forgia
2. Elementale Anziano inizia combattimento con HP ridotti (180 invece 228)
3. Permettere short rest prima Boss senza penalty tempo

**Se Party Domina Troppo Facilmente:**
1. Aggiungere Elementali Medi (2x) come rinforzi Boss Round 3
2. Boss usa tattica più intelligente (kiting, cover)
3. Hazard random durante Boss (geyser fuoco improvviso)

---

### **Foreshadowing Parte 4 (Piano Terra)**

**Durante Parte 3, Seminare Hints:**
- Schiavi menzionano "Piano Terra ancora più pericoloso - gravità schiaccia"
- Aegis Fang commenta "Prossima gemma: Smeraldo Forza. Testerà resistenza fisica."
- Topazio Visioni: Thorik vede flash cristalli viventi, peso opprimente

---

### **Emotional Beats**

**Parte 3 Deve Essere:**
- **Eroica:** Party sconfigge guardiano millenario, recupera artefatto leggendario
- **Soddisfacente:** Ricompense tangibili (tesoro, XP, Topazio)
- **Urgente:** Countdown Hammerfist sempre presente (Affresco A7 aggiorna)
- **Preparatoria:** Setup Parte 4 (Portale Terra appare)

**Climax Narrativo:** Momento Topazio manifesta (pause drammatica, musica epica)

---

## 15. STATISTICHE PARTY POST-PARTE 3

### **Aggiornamenti dopo Long Rest**

**Tutti PG:**
- **HP:** Completi (145 Thorik, 110 Tordek, 70 Artemis)
- **Benedizioni:** Rinnovate (tutte 24h duration)
- **Capacità Giornaliere:** Reset

**Thorik Specifico:**
- **Corona:** +1 gemma (Topazio Tempo attivato)
  - Nuovi poteri: Visione Temporale, Rallentare Tempo 1/giorno
- **Equipaggiamento Possibile:** Amuleto Protezione +1 (se slot libero)

**Risorse Disponibili:**
- Vigore Forgia: 3/3 cariche (tutti)
- Aegis Fang: 3/3 *Cure Moderate*
- Devastation Gauntlets: 4/4 cariche
- Ring Artemis: 5/5 cariche (con sinergia Corona)
- Trinità Divina: USATA, cooldown 24h (non disponibile Parte 4!)

**Countdown Hammerfist:** 4 giorni 2 ore rimanenti (~98 ore)

---

## **FINE PARTE 3 COMPLETA**

**Prossima Sessione:** **PARTE 4 - Piano Elementale della Terra + Smeraldo della Forza**

---

**Compatibilità D&D 3.5:** ✅ VERIFICATA  
**Elementali Bilanciati:** ✅ TESTATI (compensano Benedizioni)  
**Boss Fight Epica:** ✅ DETTAGLIATA  
**Rituale Topazio:** ✅ COMPLETO  
**Tesori Appropriati:** ✅ 15.950 mo totali  
**Progressione Storia:** ✅ 1/3 Gemme Recuperate  

Party è PRONTO per Piano Elementale della Terra!