...
| 🟡 | **Conformità meccanica degli statblocchi** — L1, L2, L5 fatti; L3 e L4 aperti | [RICERCA-CONFORMITA-MECCANICA-STATBLOCCHI](RICERCA-CONFORMITA-MECCANICA-STATBLOCCHI.md) §8 | `python3 scripts/genera_attributi.py --check` → **94 generati, 0 da generare** · `python3 scripts/validate_bestiario.py --rules` → **29 avvisi**, 26 dei quali `pf-dado` che registra un'arma *(misurato 2026-09-23)*: finché restano, `--rules` non si promuove a cancello <!-- attesa: 94 blocchi generati --> |
