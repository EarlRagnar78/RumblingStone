# Hobgoblin Regular (Warrior 3) [TRANSCRIBED — PCGen, L1 2026-07-08]
**Faction**: red-hand | **Role**: fodder | **Environment**: any | **CR**: 2 | **Source**: PCGen export `Bestiario/pregen-pcgen/png_La_mano_rossa_del_destino/hobgoblin regular 3 liv.htm` (build custom su MM Hobgoblin) | **Status**: transcribed-pcgen


```statblocco
gs: 2
tipo: Medium humanoid (goblinoid), Warrior 3, LE
ca: 19
ca-dettaglio: touch 11, flat-footed 18
pf: 23
pf-dado: 3d8+6
ts: Temp +6, Rifl +3, Vol +1
attributi: For 13 Des 13 Cos 14 Int 10 Sag 9 Car 8
velocita: 6 m
iniziativa: +1
attacchi:
  - Mischia spada lunga perfetta +6 (1d8+1/19-20); scudo pesante +0 (1d4+1)
voci:
  - Talenti: Allerta, Arma Focalizzata (spada lunga)
```

> [INFERRED — needs DM confirmation] `pf-dado` corretto da `scripts/conformita_statblocchi.py`: era «1d8+1», il danno di un'arma, che resta in `attacchi`. I dadi vengono dalle classi del `tipo` coi dadi SRD, il bonus da Cos 14.

> [INFERRED — needs DM confirmation] `attributi` trascritti dalla fonte citata in `Bestiario/pregen-pcgen/` da `scripts/genera_attributi.py`. Confermali o correggili.

Medium humanoid (goblinoid), Warrior 3, LE. **hp 23** (3 HD); **AC 19**, touch 11, flat-footed 18. Init +1; Vel 6 m. TS Temp +6, Rifl +3, Vol +1. BAB +3; Lotta +4.
**Mischia** spada lunga perfetta +6 (1d8+1/19-20); scudo pesante +0 (1d4+1). **Distanza** arco lungo composito (+1) +4 (1d8+1/x3). Scurovisione 18 m. Talenti: Allerta, Arma Focalizzata (spada lunga). Linguaggi: Comune, Goblin.
**Variante «Combattente GS2»** (`regolari/Hobgoblin Combattente gs2.htm`): hp 19, TS +5/+2/+2, spada +5, Volontà di Ferro al posto di Arma Focalizzata — stessa unità, equip leggermente diverso.
Notes: la schiera base della Mano Rossa (archi 00-04). Complementare a `goblin-warrior1-cr05` e `hobgoblin-sergente-cr5` già a catalogo.
